...
bool vnCamera::initialize()
{
	...
	Aspect = (float)vnMainFrame::screenWidth / (float)vnMainFrame::screenHeight;//アスペクト比(16:9)
	zNear = 1.0f;
	zFar = 1000.0f;
...
{
	View = XMMatrixLookAtLH(Position, Target, Up);//ビューマトリクスを作る関数（左手座標系(LeftHand)で作る）
	return &View;
}

XMMATRIX *vnCamera::getProj()
{
	Proj = XMMatrixPerspectiveFovLH(FovY, Aspect, zNear, zFar);//FovY, Aspect, zNear, zFarの要素をまとめている
	return &Proj;
}
