...
{
private:
	static XMVECTOR			Position;	//カメラの座標	　位置ベクトル
	static XMVECTOR			Target;		//カメラの注視点　位置ベクトル
	static XMVECTOR			Up;			//カメラの上方向　方向ベクトル

	static float			FovY;		//視野角 (こだわりポイント)
	static float			Aspect;		//アスペクト比
	static float			zNear;		//前方クリッピング面のの距離
	static float			zFar;		//後方クリッピング面の距離		

	//基本的には上記の２要素でカメラを設定している

	static XMMATRIX			View;		//ビュー行列　上１つ目のグループをまとめている
	static XMMATRIX			Proj;		//プロジェクション行列　上２つ目のグループをまとめている
